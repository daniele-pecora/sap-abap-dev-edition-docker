...
#-----------
    ...
    #MOD: Remove installation flag file
    rm -f /tmp/sapdownloads/installed
    echo "*** MOD: Remove installation flag file: /tmp/sapdownloads/installed"
...
#-----------------------------------------------------------
    ...
    # MOD: skip this by don't asking for licence aggreement
    #ask_which_license
    echo "*** MOD: We automatically agree to license"
    ...
    # MOD: skip this by setting pwd fixed
    #ask_for_password masterpwd
    export masterpwd="Down1oad"
    echo "*** MOD: We passed fixed password: ${masterpwd}"
    ...
    echo $tasks | grep -q $TASK_extract && {
        #MOD:skip this if already extracted (this is the last file from last server tar)
        if [ ! -f "/sapmnt/NPL/profile/NPL_D00_vhcalnplci" ];then
            extract_tar
            #MOD:after extraction set the T11889 flag to file (https://blogs.sap.com/2021/06/07/adjusting-installer-script-for-sap-netweaver-dev-edition-for-distros-with-kernel-version-5.4-or-higher/)
            sed -i 's/NPL.cfg \\/NPL.cfg -T11889 \\/g' /sybase/NPL/ASE-16_0/install/RUN_NPL
            echo "*** MOD: Added flat to support kernel higher 5.4.x '-T11889' into /sybase/NPL/ASE-16_0/install/RUN_NPL"
        else
            echo "*** MOD: Re-use already extracted installation files."
        fi
    }
    ...
    echo " "
    #MOD: Create a flag file to check if installation has been completed successfully
    echo "Installation completed successfully - `date`" > /tmp/sapdownloads/installed
    echo "*** MOD: created installation completed successfully flag file: /tmp/sapdownloads/installed"
    source /tmp/sapdownloads/instructions
    exit 0
}
